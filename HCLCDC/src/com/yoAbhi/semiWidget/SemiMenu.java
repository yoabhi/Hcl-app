package com.yoAbhi.semiWidget;

import java.util.HashMap;


import com.yoAbhi.semiWidget.SemiMenuItem;
import com.yoAbhi.radialmenu.RadialMenuColors;
...
public class SemiMenu extends View{
	...
	//Static Access Variables
		...
		private HashMap<String, SemiMenuItem> mMenuItems = new HashMap<String, SemiMenuItem>();
		...
		private int mShadowColor = Color.GREEN;	
		...
		private int mToggleMenuTextColor = Color.BLACK;
		private float textSize = 12 * getResources().getDisplayMetrics().density;
		private int mOpenButtonScaleFactor = 3;
			
	
	public SemiMenu(Context context) {
		super(context);
		
		init();
	}

	public SemiMenu(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isMenuVisible) {
			...
			if(mMenuItems.size() > 0) {
				...
				for(SemiMenuItem item : mMenuItems.values()) {
	 ...
	 */
	public void addMenuItem(String idTag, SemiMenuItem mMenuItem) {
